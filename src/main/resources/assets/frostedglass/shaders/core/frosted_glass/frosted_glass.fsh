...
void main() {
    vec2 cord = ((texCoord0 * positionWInv) + vec2(1.0, 1.0)) / 2.0;
    vec2 tFrag = vec2(1.0 / 500.0);
    vec4 destColor = texture(Sampler0, cord);// * vertexColor;// * ColorModulator;
    ...
    //    vec2 cord = ((texCoord0 * positionWInv) + vec2(1.0, 1.0)) / 2.0;
    //    vec4 color = texture(Sampler0, cord);// * vertexColor;
    //    fragColor = color;
}
